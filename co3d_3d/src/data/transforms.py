...
##############################
@gin.configurable
class RandomFeatureJitter(object):
    def __init__(
        self, std=0.01, application_ratio=0.9, start_ind: int = 4, feature_dim: int = 27
    ):
        self.std = std
    ...
    def __call__(self, coords, feats, labels):
        if random.random() < self.application_ratio:
            noise = np.random.randn(feats.shape[0], self.feature_dim) - 0.5
            noise *= self.std
            end_ind = self.start_ind + self.feature_dim
            feats[:, self.start_ind : end_ind] += noise
...
class RandomCrop(object):
    ...
    def __call__(self, coords: np.array, feats, labels):
        ...
        while not valid:
            ...
            if retries % 2 == 0:
                logging.warn(
                    f"RandomCrop retries: {retries}. crop_range=from {min_box} to {max_box}"
                )
...
class FarthestPointSample(object):
    def __init__(self, num_points) -> None:
        ...
        logging.info(f"{self.__class__.__name__} with num_points: {num_points}")
    ...
    def sample(self, points):
        ...
        N, D = points.shape
        xyz = points[:, :3]
        centroids = torch.zeros((self.num_points,))
        ...
        logging.debug(
            f"sample with num_points {self.num_points}: {points.shape[0]} to {centroids.shape[0]}"
        )
        return centroids.long()

    def __call__(self, coords, feats, labels):
        centroids = self.sample(coords)
        return coords[centroids], feats[centroids], labels
...
class RandomSample(object):
    ...
    def sample(self, points):
        if points.shape[0] > self.num_points:
            ind = np.random.choice(points.shape[0], self.num_points, replace=False)
        else:
            ind = np.random.choice(points.shape[0], self.num_points, replace=True)
        logging.debug(
            f"sample with num_points {self.num_points}: {points.shape[0]} to {ind.shape[0]}"
        )
        return ind

    def __call__(self, coords, feats, labels):
        ind = self.sample(coords)
        return coords[ind], feats[ind], labels


@gin.configurable()
class DensityBasedSample(object):
    def __init__(self, percentile, density_dim: int = 3) -> None:
        self.percentile = percentile
        self.density_dim = density_dim
        assert (
            self.density_dim > 0
        ), f"density_dim should be larger than 0, but got {density_dim}"
        logging.info(f"{self.__class__.__name__} with percentile : {percentile}")

    def sample(self, density):
        # ths = torch.quantile(density, self.percentile)
        ths = np.percentile(density, self.percentile)
        sel = density > ths
        logging.debug(
            f"sample with percentile {self.percentile}: {density.shape[0]} to {sel.sum().item()}"
        )
        return sel

    def __call__(self, coords, feats, labels):
        assert (
            feats.shape[1] > self.density_dim
        ), f"{self.density_dim} out of feature dimension({feats.shape[1]})"
        sel = self.sample(feats[:, self.density_dim])
        return (
            coords[sel],
            feats if feats is None else feats[sel],
            labels if labels is None else labels[sel],
...
@gin.configurable()
class LabelBasedSample(object):
    def __init__(self, filter_label) -> None:
        self.filter_label = filter_label
        logging.info(f"{self.__class__.__name__} with filter_label: {filter_label}")

    def sample(self, labels):
        sel = labels == self.filter_label
        logging.debug(
            f"sample with filter_label {self.filter_label}: {labels.shape[0]} to {sel.sum()}"
        )
        return sel

    def __call__(self, coords, feats, labels):
        assert (
            coords.shape[0] == labels.shape[0]
        ), f"labels should have same length with coordinates"
        sel = self.sample(labels)
        return (
            coords[sel],
            feats if feats is None else feats[sel],
            labels if labels is None else labels[sel],
...
class pt_limited_collate_fn_factory:
