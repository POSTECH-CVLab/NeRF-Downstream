...
from co3d_3d.src.models.paconv.cuda_lib.functional import (
    assign_score_withk as assemble_dgcnn,
)
...
class PAConv(nn.Module):
    def __init__(self, args):
        ...
        tensor1 = (
            nn.init.kaiming_normal_(
                torch.empty(self.m1, i1 * 2, o1), nonlinearity="relu"
            )
            .permute(1, 0, 2)
            .contiguous()
            .view(i1 * 2, self.m1 * o1)
        )
        tensor2 = (
            nn.init.kaiming_normal_(
                torch.empty(self.m2, i2 * 2, o2), nonlinearity="relu"
            )
            .permute(1, 0, 2)
            .contiguous()
            .view(i2 * 2, self.m2 * o2)
        )
        tensor3 = (
            nn.init.kaiming_normal_(
                torch.empty(self.m3, i3 * 2, o3), nonlinearity="relu"
            )
            .permute(1, 0, 2)
            .contiguous()
            .view(i3 * 2, self.m3 * o3)
        )
        tensor4 = (
            nn.init.kaiming_normal_(
                torch.empty(self.m4, i4 * 2, o4), nonlinearity="relu"
            )
            .permute(1, 0, 2)
            .contiguous()
            .view(i4 * 2, self.m4 * o4)
        )
        ...
        self.conv5 = nn.Sequential(
            nn.Conv1d(512, 1024, kernel_size=1, bias=False), self.bn5
        )
    ...
    def forward(self, x, label=None, criterion=None):
        ...
        point1, center1 = feat_trans_dgcnn(
            point_input=x, kernel=self.matrice1, m=self.m1
        )  # b,n,m1,o1
        score1 = self.scorenet1(xyz, calc_scores=self.calc_scores, bias=0.5)
        """assemble with scores:"""
        point1 = assemble_dgcnn(
            score=score1,
            point_input=point1,
