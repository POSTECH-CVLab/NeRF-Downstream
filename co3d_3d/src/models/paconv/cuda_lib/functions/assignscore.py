...
class AssignScoreWithKHalfKernel(Function):
    ...
    def forward(ctx, scores, points, knn_idx, aggregate):  # -> torch.Tensor:
        ...
        src.gpu.assign_score_withk_halfkernel_forward_cuda(
            B,
            N,
            M,
            K,
    ...
    def backward(ctx, grad_out):
        ...
        grad_points = torch.zeros_like(
            points, dtype=points.dtype, device=points.device
        ).contiguous()
        grad_scores = torch.zeros_like(
            scores, dtype=scores.dtype, device=scores.device
        ).contiguous()

        src.gpu.assign_score_withk_halfkernel_backward_cuda(
            B,
            N,
            M,
            K,
...
class AssignScoreWithK(Function):
    ...
    def forward(ctx, scores, points, centers, knn_idx, aggregate):  # -> torch.Tensor:
        ...
        src.gpu.assign_score_withk_forward_cuda(
            B,
            N,
            M,
            K,
    ...
    def backward(ctx, grad_out):
        ...
        grad_centers = torch.zeros_like(
            centers, dtype=points.dtype, device=points.device
        ).contiguous()
        grad_scores = torch.zeros_like(
            scores, dtype=scores.dtype, device=scores.device
        ).contiguous()

        src.gpu.assign_score_withk_backward_cuda(
            B,
            N,
            M,
            K,
