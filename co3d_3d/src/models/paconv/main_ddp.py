...
def get_logger():
    ...
    file_handler = logging.FileHandler(
        os.path.join(
            "checkpoints", args.exp_name, "main-" + str(int(time.time())) + ".log"
        )
    )
...
def get_parser():
    ...
    parser.add_argument(
        "--config", type=str, default="config/dgcnn_paconv.yaml", help="config file"
    )
    parser.add_argument(
        "opts",
        help="see config/dgcnn_paconv.yaml for all options",
        default=None,
...
def train(gpu, ngpus_per_node):
    ...
    # =========== Dataloader =================
    train_data = ModelNet40(
        partition="train", num_points=args.num_points, pt_norm=args.pt_norm
    )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if test_acc >= best_test_acc and main_process():
            ...
            torch.save(
                model.state_dict(), "checkpoints/%s/best_model.t7" % args.exp_name
            )  # save the best model
...
def train_epoch(train_loader, model, opt, scheduler, epoch, criterion):
    ...
    for ii, (data, label) in enumerate(train_loader):
        ...
        data, label = (
            ...
            label.cuda(non_blocking=True).squeeze(1),
        )
        ...
        if args.multiprocessing_distributed:
            ...
            _count = label.new_tensor([batch_size], dtype=torch.long).cuda(
                non_blocking=True
            )  # b_size on one process
            dist.all_reduce(loss), dist.all_reduce(
                _count
            )  # obtain the sum of all xxx at all processes
            n = _count.item()
        ...
        ):  # obtain the sum of all tensors at all processes: all_reduce
...
def test(gpu, ngpus_per_node):
    ...
    state_dict = torch.load(
        "checkpoints/%s/best_model.t7" % args.exp_name, map_location=torch.device("cpu")
    )
    ...
    if main_process():
        ...
        for i in range(args.classes):
            logger.info(
                "Class_{} Result: accuracy {:.4f}.".format(i, accuracy_class[i])
            )
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if main_process():
        ...
        if not args.eval:  # backup the running files
            ...
            os.system(
                "cp util/PAConv_util.py checkpoints"
                + "/"
                + args.exp_name
                + "/"
                + "PAConv_util.py.backup"
            )
            os.system(
                "cp util/data_util.py checkpoints"
                + "/"
                + args.exp_name
                + "/"
                + "data_util.py.backup"
            ...
            if args.arch == "dgcnn":
                os.system(
                    "cp model/DGCNN_PAConv.py checkpoints"
                    + "/"
                    + args.exp_name
                    + "/"
                    + "DGCNN_PAConv.py.backup"
                )
            elif args.arch == "pointnet":
                os.system(
                    "cp model/PointNet_PAConv.py checkpoints"
                    + "/"
                    + args.exp_name
                    + "/"
                    + "PointNet_PAConv.py.backup"
                )
    ...
    assert not args.eval, (
        "The all_reduce function of PyTorch DDP will ignore/repeat inputs "
